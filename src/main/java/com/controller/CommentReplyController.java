...
public class CommentReplyController {
    ...
    public ResultVo queryCommentReply(@PathVariable("commid") String commid){
        ...
        for (Comment comment : commentsList) {
            ...
            /**查询评论者的昵称和头像信息*/
            System.out.println("コメントID：" + comment.getCuserid());
        ...
        return new ResultVo(true, StatusCode.OK,"問い合わせコメントの返信は成功しました",commentsList);
    }
    ...
    public ResultVo insertcomment(@RequestBody Comment comment,HttpSession session) {
        ...
        if (StringUtils.isEmpty(cuserid)) {
            return new ResultVo(false,StatusCode.ACCESSERROR,"ログインしてからコメントしてください");
        }
        ...
        if (i == 1){
            ...
            Notices notices = new Notices().setId(KeyUtil.genUniqueKey()).setUserid(comment.getSpuserid()).setTpname("コメント")
                    .setWhys("あなたの商品 <a href=/product-detail/"+comment.getCommid()+" style=\"color:#08bf91\" target=\"_blank\" >"+commodity.getCommname()+"</a> コメントされましたので、早く行ってみてください。");
            noticesService.insertNotices(notices);
            return new ResultVo(true, StatusCode.OK,"コメント成功");
        }
        return new ResultVo(false,StatusCode.ERROR,"コメント失敗");
    }
    ...
    public ResultVo insertreply(@RequestBody Reply reply,HttpSession session) {
        ...
        if (i == 1){
            ...
            Notices notices = new Notices().setId(KeyUtil.genUniqueKey()).setUserid(reply.getCuserid()).setTpname("コメント返信")
                    .setWhys("誰かがあなたの販売商品 <a href=/product-detail/"+reply.getCommid()+" style=\"color:#08bf91\" target=\"_blank\" >"+commodity.getCommname()+"</a> であなたのコメントに返事をしましたので、早く行ってみてください。。");
            noticesService.insertNotices(notices);
            return new ResultVo(true, StatusCode.OK,"返事が成功した");
        }
        return new ResultVo(false,StatusCode.ERROR,"返事が失敗した");
    }
    ...
    public ResultVo deletecomment(@PathVariable("cid") String cid,HttpSession session){
        ...
        if (comment.getCuserid().equals(cuserid) || comment.getSpuserid().equals(cuserid)){
            ...
            if (i == 1 && j == 1){
                return new ResultVo(true, StatusCode.OK,"削除成功");
            }
            return new ResultVo(false, StatusCode.ERROR,"削除失敗");
        }
        return new ResultVo(false,StatusCode.ACCESSERROR,"操作を禁止する");
    }
    ...
    public ResultVo deletereply(@PathVariable("rid") String rid,HttpSession session){
        ...
        if (reply.getRuserid().equals(ruserid) || reply.getSpuserid().equals(ruserid)){
            ...
            return new ResultVo(false, StatusCode.ERROR,"削除に失敗した");
        }
        return new ResultVo(false,StatusCode.ACCESSERROR,"操作を禁止する");
    }
