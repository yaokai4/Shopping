    ...
    public ResultVo insertcollect(@RequestBody Collect collect, HttpSession session){
        ...
        if (colloperate == 1){
            ...
            if(!StringUtils.isEmpty(collect1)){
                ...
                if (i == 1){
                    return new ResultVo(true, StatusCode.OK,"コレクションに成功する");
                }
                return new ResultVo(false,StatusCode.ERROR,"コレクションに失敗する");
            }else{
                collect.setId(KeyUtil.genUniqueKey());
                Integer i = collectService.insertCollect(collect);
                if (i == 1){
                    return new ResultVo(true, StatusCode.OK,"コレクションに成功");
                }
                return new ResultVo(false,StatusCode.ERROR,"コレクションに失敗");
            }
            ...
            if (collect1.getCouserid().equals(couserid)){
                Integer i = collectService.updateCollect(collect);
                if (i == 1){
                    return new ResultVo(true, StatusCode.OK,"キャンセル成功");
                }
                return new ResultVo(false,StatusCode.ERROR,"キャンセル失敗");
            }
