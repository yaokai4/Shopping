...
public class ChatCtrl {
    ...
    public JSONObject upimg(@RequestParam(value = "file", required = false) MultipartFile file) throws IOException {
        ...
        file.transferTo(new File("//Users//yaokai//Desktop//IT//xiangmuhzaopian//" + filenames));
    ...
    public ResultVo addfrend(@PathVariable("fuserid") String fuserid,HttpSession session){
        ...
        if(userid.equals(fuserid)){
            //不能对自己的商品感兴趣
           return new ResultVo(false, StatusCode.ERROR,"自分の商品に興味を持ってはいけません");
        }
        ...
        return new ResultVo(false, StatusCode.OK,"チャット画面にジャンプしています");
    }
    ...
    public InitImVo initim(HttpSession session){
        ...
        Friend friend=new Friend().setId("2").setGroupname("組分け").setList(list);
