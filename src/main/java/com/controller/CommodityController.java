...
public class CommodityController {
    ...
    public String changegoods(@RequestBody Commodity commodity, HttpSession session){
        ...
        /**发出待审核系统通知*/
        Notices notices = new Notices().setId(KeyUtil.genUniqueKey()).setUserid(userid).setTpname("商品審査")
                .setWhys("あなたの商品 <a href=/product-detail/"+commodity.getCommid()+" style=\"color:#08bf91\" target=\"_blank\" >"+commodity.getCommname()+"</a> 審査待ち列に入りますので、お待ちください。");
    ...
    public String relgoods(@RequestBody Commodity commodity, HttpSession session){
        ...
        /**发出待审核系统通知*/
        Notices notices = new Notices().setId(KeyUtil.genUniqueKey()).setUserid(userid).setTpname("商品審査")
                .setWhys("あなたの商品 <a href=/product-detail/"+commid+" style=\"color:#08bf91\" target=\"_blank\" >"+commodity.getCommname()+"</a> 審査待ち列に入りますので、お待ちください。");
        noticesService.insertNotices(notices);
        return "0";
    }
    ...
    public PageVo searchCommodityNumber(@PathVariable("commname") String commname){
        ...
        return new PageVo(StatusCode.OK,"照会が成功した",dataNumber);
    }
    ...
    public ResultVo searchCommodity(@PathVariable("nowPaging") Integer page, @PathVariable("commname") String commname){
        ...
        if(!StringUtils.isEmpty(commodityList)){//如果有对应商品
            ...
            return new ResultVo(true,StatusCode.OK,"照会が成功した",commodityList);
        }else{
            return new ResultVo(true,StatusCode.ERROR,"関連商品はありません");
        }
    }
