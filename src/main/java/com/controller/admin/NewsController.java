...
public class NewsController {
    ...
    public ResultVo insertNews(@RequestBody News news, HttpSession session){
        ...
        if (i == 1){
            return new ResultVo(true, StatusCode.OK,"公告発表に成功する");
        }
        return new ResultVo(false,StatusCode.ERROR,"公告発表に失敗しましたので、再発行してください");
    }
    ...
    public ResultVo delectNews (@PathVariable("id") String id, HttpSession session) {
        ...
        if (StringUtils.isEmpty(news)){
            return new ResultVo(false,StatusCode.FINDERROR,"削除の公告が見つからない");
        }else {
            /**判断是否是本人或超级管理员*/
            if (news.getUsername().equals(username) || username.equals("admin")){
                ...
                return new ResultVo(false,StatusCode.ACCESSERROR,"権限が不足して削除できない");
            }
        }
    }
    ...
    public ResultVo updateNews (@RequestBody News news){
        ...
        if (i == 1){
            return new ResultVo(true,StatusCode.OK,"修正が成功する");
        }
        return new ResultVo(false,StatusCode.ERROR,"修正に失敗する");
    }
    ...
    public ResultVo queryNews (){
        ...
        return new ResultVo(true,StatusCode.OK,"照会が成功した",newslist);
    }
