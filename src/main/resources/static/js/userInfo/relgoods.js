...
layui.use(['form', 'upload', 'element'], function () {
    ...
    upload.render({
        ...
        , done: function (res) {
            //如果上传失败
            if (res.code > 0) {
                return layer.msg('アップロードに失敗した');
            } else {
                layer.closeAll('loading');
                layer.msg('アップロードに成功した', {
                    time: 1000,
                    icon: 1,
                    offset: '150px'
                });
    ...
    form.on('submit(demo1)', function (data) {
        ...
        if(vuemainimg.length===0){
            layer.msg('商品のメインマップをアップロードしてください', {
                time: 1000,
                icon: 2,
                offset: '150px'
            });
